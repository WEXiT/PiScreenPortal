...
(function () {
  ...
  async function handle(url, init) {
    ...
    function buildMaintenance() {
      const ar = state.cfg.auto_reboot || {};
      let next = null;
      if (ar.enabled) {
        const now = new Date();
        if (ar.mode === "interval") {
          next = new Date(now.getTime() + (Number(ar.interval_minutes) || 1440) * 60000);
        } else {
          const [h, m] = String(ar.time || "06:00").split(":").map(x => parseInt(x, 10) || 0);
          next = new Date(now);
      ...
      return {
        auto_reboot: ar,
        running: true,
        next_reboot_at: next ? next.toISOString().slice(0, 19).replace("T", " ") : null,
        next_reboot_in_seconds: next ? Math.floor((next - Date.now()) / 1000) : null,
        last_auto_reboot_at: null,
        version: state.version,
        git: {
          available: true,
          remote: "https://github.com/WEXiT/PiScreenPortal.git",
          branch: "main",
          head: "demo",
        },
      };
    ...
    if (path === "/api/maintenance/status") {
      return json(buildMaintenance());
    }
    if (path === "/api/maintenance/check-updates" && method === "POST") {
      return json({
        ok: true,
        current_version: state.version,
        remote_version: state.remoteVersion,
        update_available: Number(state.remoteVersion) > Number(state.version),
        local_newer: Number(state.version) > Number(state.remoteVersion),
        current_head: "demo1",
        remote_head: "demo2",
    ...
    if (path === "/api/maintenance/update" && method === "POST") {
      ...
      state.version = state.remoteVersion;
      log("git update + reboot scheduled (demo)");
      return json({
        ok: true,
        before: "demo1",
        after: "demo2",
    ...
    if (path === "/api/maintenance/optimize-boot" && method === "POST") {
      log("boot view optimized (demo)");
      return json({
        ok: true,
        steps: [
          {step: "assets", ok: true, msg: "OK"},
          {step: "pcmanfm", ok: true, msg: "OK"},
          {step: "lxpanel", ok: true, msg: "OK"},
          {step: "autostart", ok: true, msg: "OK"},
          {step: "wayland-autostart", ok: true, msg: "OK"},
          {step: "keyring", ok: true, msg: "OK"},
        ],
      });
    }
